<template>
  <div class="flex items-center justify-between my-5 px-4 lg:px-0">
    ...
    <div>
      <button class="bg-main text-white p-2 px-4 rounded-lg">Корзина <span v-if="orders != null" class="text-black font-bold">{{ orders[0].goods.length }}</span></button>
      <button v-if="currentUser === null" @click="showModalReg = 1" class="py-2 px-4 rounded-full bg-gray-200 hover:bg-main hover:text-white ml-4">Зарегистрироваться</button>
